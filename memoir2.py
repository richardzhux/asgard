...
# "Units" = English words or single CJK characters
CHUNK_WORDS = 1500       # target "units" per chunk
CHUNK_OVERLAP = 150      # overlap to avoid slicing scenes mid-thought

# Output token limits, sized closer to real needs
CHUNK_MAX_OUTPUT_TOKENS = 2000       # generous headroom for the per-chunk schema
SYNTHESIS_MAX_OUTPUT_TOKENS = 16000  # plenty for a 5k+ word dossier
...
# Per-model token-per-minute limits
# Your current quota: ~1,000,000 tokens/min, 5,000 RPM
# We only model TPM here; RPM is unlikely to be hit in this pipeline.
MODEL_LIMITS = {
    MODEL_SUMMARY:   {"tpm": 1_000_000},
    MODEL_SYNTHESIS: {"tpm": 1_000_000},
}
...
class MemoirChunk:
    text: str
    start_unit: int  # 1-based "unit index" (English word or CJK char)
    end_unit: int    # inclusive
    idx: int
...
def rebuild_text(tokens: List[str]) -> str:
    ...
    NOTE: This is an approximate reconstruction, not a byte-perfect copy.
    """
    if not tokens:
        return ""
...
def chunk_text_by_words(text: str, max_words: int, overlap: int) -> List[MemoirChunk]:
    ...
    Returns MemoirChunk objects with unit index ranges (1-based start).
    """
    ...
    while start < total_units:
        ...
        chunks.append(
            MemoirChunk(
                text=chunk_text,
                start_unit=start + 1,  # 1-based index for readability
                end_unit=end,
                idx=idx,
            )
        )
...
def chunk_summary_path(idx: int) -> Path:
    # human-friendly 1-based naming
    return SUMMARIES_DIR / CHUNK_FILENAME_TEMPLATE.format(idx + 1)


def compute_index_meta(full_text: str) -> dict:
    """
    Fingerprint to ensure index + summaries actually match the current memoir and config.
    If you change the file, chunking params, or models, we'll refuse to reuse old summaries.
    """
    return {
        "memoir_path": str(Path(MEMOIR_PATH).resolve()),
...
def load_or_init_index(chunks: List[MemoirChunk], full_text: str) -> dict:
    ensure_summaries_dir()
    meta = compute_index_meta(full_text)
    ...
    if INDEX_PATH.exists():
        ...
        if len(data.get("chunks", [])) != len(chunks):
            raise RuntimeError(
                ...
                "Delete summaries/index.json (and summaries/*.md) to re-run fresh."
            )

        old_meta = data.get("meta")
        if old_meta != meta:
            raise RuntimeError(
                "Index metadata does not match current memoir/config.\n"
                "Likely causes:\n"
                "- Memoir file changed\n"
                "- CHUNK_WORDS / CHUNK_OVERLAP changed\n"
                "- Model names changed\n"
                "Delete summaries/index.json (and summaries/*.md) to re-run fresh."
            )
...
# Classify OpenAI exceptions in a version-tolerant way
...
def call_model(
    ...
    last_exc = None
    transient_errors = OPENAI_TRANSIENT_ERRORS + (TimeoutError,)  # builtin TimeoutError as fallback
    ...
    for attempt in range(max_retries):
        try:
        ...
        except OPENAI_HARD_ERRORS as e:  # non-retryable
            raise RuntimeError(f"{call_context}: non-retryable OpenAI API error: {e!r}") from e
            ...
            # Unknown error: treat as transient but still bounded by max_retries
            last_exc = e
            backoff = min(2 ** attempt, 60)
            print(
...
def analyze_chunk(chunk: MemoirChunk) -> str:
    idx = chunk.idx
    chunk_header = f"(chunk {idx+1}, units {chunk.start_unit}-{chunk.end_unit})\n\n{chunk.text}"
    ...
    header = f"# Chunk {idx+1} (units {chunk.start_unit}-{chunk.end_unit})\n\n"
...
def main():
    ...
    print(
        f"Loaded memoir with {total_units} units into {total_chunks} chunks "
        f"(overlap {CHUNK_OVERLAP})."
    )

    index_data = load_or_init_index(chunks, full_text)
    ...
    for chunk in chunks:
        ...
        if entry["status"] == "done" and summary_path.exists():
            ...
            duration_note = f" (took {duration:.1f}s)" if duration is not None else ""
            print(
                f"\nChunk {chunk.idx+1}/{total_chunks} already done{duration_note}; skipping."
            )
            render_progress(done_so_far, total_chunks)
            continue

        print(
            f"\nAnalyzing chunk {chunk.idx+1}/{total_chunks} "
            f"(units {chunk.start_unit}-{chunk.end_unit})..."
        )
        ...
        done_so_far += 1
        print(
            f"Finished chunk {chunk.idx+1}/{total_chunks} | "
            f"{duration:.1f}s (logged)"
        )
        render_progress(done_so_far, total_chunks)
